...
const ProjectSection = () => {
  ...
  return <section id="projects" className="py-20 bg-subtle-gradient">
      <div className="container mx-auto px-6">
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-7xl mx-auto">
          {projects.map((project, index) => <div key={project.id} className="scroll-reveal group cursor-pointer" style={{
              ...
              <div className="relative overflow-hidden h-64 rounded-3xl shadow-xl mb-8 group-hover:shadow-2xl transition-all duration-500 group-hover:scale-105">
                ...
                <div className="absolute bottom-4 left-4 right-4 transform translate-y-4 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-500">
                  <div className="flex flex-wrap gap-2">
                    {project.tech.map((tech, techIndex) => <span key={techIndex} className="px-3 py-1 bg-white/20 backdrop-blur-sm text-white text-xs rounded-full border border-white/30">
                        {tech}
                      </span>)}
                  </div>
                </div>
